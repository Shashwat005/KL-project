    ...
    cy.url().should('include','/home')
              ...
              /*  cy.get('body').then((body) => {
                if (body.find('wrongLocator').length > 0) {
                    ...
                    cy.get('.py-24').click()
                       ...
                       cy.contains('Cancel').should('be.visible').click() }                       //TODO: Cancel button is missing
                })
        ...
        it('New Account',()=> {
          ...
          */    cy.get('.gap-3 > :nth-child(1) > .flex > .w-full').type('abc')      //Comapny Name
              cy.get('.gap-3 > :nth-child(2) > .flex > .w-full').type('Test 1')
              cy.wait(8000)
              cy.get(':nth-child(3) > .rounded-md').type('424242424242424242424242')        // TODO:Manually enter card no.
              cy.get(':nth-child(2) > .transition-colors').click()
             // cy.contains('Account added successfully').should('be.visible')                  //TODO:
              cy.url().should('include','accounts')
          ...
          cy.contains('Active Accounts', { timeout: 10000 }).should('be.visible');  
           ...
           cy.get('[href="/accounts"] > .relative').click()                               //Account back button verify
           cy.url().should('include','https://accounts.kloudlite.io/accounts')
               ...
               cy.get('.gap-3 > :nth-child(1) > .flex > .w-full').type('njiop')                                     //Name
               cy.get('.gap-3 > :nth-child(2) > .flex > .w-full').type('njiop-3456')                                    //Project id
               ...
               cy.contains('njiop').should('be.visible')
               cy.url().should('include','projects')
